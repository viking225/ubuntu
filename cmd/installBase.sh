...
direct_install(){
    ...
    my_echo "Installing $name" 1

    if [ -f /usr/local/bin/$name ]; then
        echo "Already installed"
    fi

    dl $link $path && sudo mv $path /opt/$name || my_echo "Download failed" 2 && return 1
    chmod +x $name
    sudo ln -s /opt/$name /usr/local/bin/$name
}
...
dpkg_install(){
    ...
    if [ -d /opt/$name ]; then
        my_echo "Already installed" 2 && return 1
    fi
...
opt_install(){
    ...
    [[ "$strip" = true ]] && tar --strip-components=1 -xvf $path -C /opt/$name || tar -xvf $path -C /opt/$name
...
dl(){
    ...
    wget -q --show-progress $link --output-document=$path
    return $?
}
...
#Docker
dpkg_install 'https://download.docker.com/linux/ubuntu/dists/xenial/pool/stable/amd64/docker-ce_18.03.1~ce-0~ubuntu_amd64.deb' docker

direct_install "https://github.com/docker/compose/releases/download/1.21.1/docker-compose-$(uname -s)-$(uname -m)" docker-compose

#Robomongo
opt_install https://download.robomongo.org/1.2.1/linux/robo3t-1.2.1-linux-x86_64-3e50a65.tar.gz robo3t true
...
opt_install https://download.jetbrains.com/webstorm/WebStorm-2018.1.2.tar.gz webstorm true
